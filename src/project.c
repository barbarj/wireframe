...
void					fdf_proj(t_fdf_matrix *matrix)
{
	...
	scale_factor = (FDF_WINWIDTH * 7.5) / (matrix->width * FDF_POINT_SEP * 10.0);
	offset[0] = (FDF_WINWIDTH / 2) - (matrix->width * FDF_POINT_SEP * scale_factor / 2);
	...
	while (h < matrix->height)
	{
		...
		{
			matrix->points2d[h][w] = calc_iso_proj(matrix->points3d[h][w], scale_factor);
